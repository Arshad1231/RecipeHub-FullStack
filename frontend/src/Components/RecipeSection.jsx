  ...
  const RecipeSection = ({ProfileDetails}) => {
    ...
    return (
      <div >
        <h1 className='font-submitted-recipes text-center text-4xl mt-5 bg-amber-500 text-white rounded-xl p-1'>Submitted Recipes!</h1>
        {UserRecipes.map((EachRecipe,index)=>(
          <div className='flex flex-col justify-between mt-3 shadow-lg p-6 rounded-xl' key={index}>
            <div className='flex flex-col'>
              <h1 className='font-Login-text text-4xl'>{EachRecipe.RecipeName}</h1>
            ...
            <div className='flex flex-row gap-5'>
              <button type="button" className="font-HomePage-text bg-amber-500 rounded-3xl p-1 w-30 h-12 flex items-center justify-center mt-2 text-white hover:bg-blue-500">
                <Link to={`/details/${EachRecipe._id}`}>Get Details</Link>
              </button>
              <button type="button" className="font-HomePage-text bg-amber-500 rounded-3xl p-1 w-30 h-12 flex items-center justify-center mt-2 text-white hover:bg-blue-500">
                <Link to={`/edit/${EachRecipe._id}`}>Edit</Link>
              </button>
              <button type="button" onClick={(e)=>HandleDelete(EachRecipe)} className='font-HomePage-text bg-red-500 rounded-3xl p-1 w-30 h-12 flex items-center justify-center mt-2 text-white hover:bg-red-200'>Delete</button>
